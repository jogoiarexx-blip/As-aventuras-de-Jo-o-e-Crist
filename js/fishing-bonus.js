...
(() => {
  ...
  class FishingBonusController{
    ...
    drawChico(ctx){
      const frames=(typeof CHICO_FRAMES!=='undefined'&&CHICO_FRAMES.fishing)||[];
      const img=frames.length?frames[Math.floor(performance.now()/180)%frames.length]:chico;
      if(img?.complete&&img.naturalWidth){ctx.save();ctx.imageSmoothingEnabled=false;ctx.drawImage(img,55,348,115,150);ctx.restore();}
    ...
    drawFight(ctx){
      ...
      this.drawBackdrop(ctx);if(this.chicoNpc)this.chicoNpc.victory=true;this.drawChicoNpc(ctx);this.players.forEach(p=>this.drawScenePlayer(ctx,p));
      ...
      if(s.phase===2&&s.life>0){const pulse=.12+.08*Math.sin(performance.now()*.01);ctx.save();ctx.globalAlpha=pulse;ctx.fillStyle='#ff3b2f';ctx.fillRect(0,0,W,H);ctx.restore();}
    }
